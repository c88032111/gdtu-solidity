...
`the release changelog <https://github.com/c88032111/solidity/releases/tag/v0.8.0>`_.
...
* Explicit conversion into ``address`` type always returns a non-payable ``address`` type. In
  ...
  - ``address(c)`` where ``c`` is a contract. Previously, the return type of this
    conversion depended on whether the contract can receive Gdtuer (either by having a receive
    function or a payable fallback function). The conversion ``payable(c)`` has the type ``address
    payable`` and is only allowed when the contract ``c`` can receive Gdtuer. In general, one can

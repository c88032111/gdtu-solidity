...
`Access Remix online <https://remix.gdtucoin.org/>`_, you do not need to install anything.
If you want to use it without connection to the Internet, go to
https://github.com/c88032111/remix-live/tree/gh-pages and download the ``.zip`` file as
...
`repository <https://github.com/c88032111/solc-js>`_.
...
.. note::
    ...
    The commandline options of ``solcjs`` are not compatible with ``solc`` and tools (such as ``ggdtu``)
    expecting the behaviour of ``solc`` will not work with ``solcjs``.

Docker
======

Docker images of Solidity builds are available using the ``solc`` image from the ``gdtucoin`` organisation.
...
.. code-block:: bash
    ...
    docker run -v /local/path:/sources gdtucoin/solc:stable -o /sources/output --abi --bin /sources/Contract.sol
...
.. code-block:: bash
    ...
    docker run gdtucoin/solc:stable --standard-json < input.json > output.json

Linux Packages
==============

Binary packages of Solidity are available at
`solidity/releases <https://github.com/c88032111/solidity/releases>`_.
...
.. code-block:: bash
    ...
    sudo add-apt-repository ppa:gdtucoin/gdtucoin
    sudo apt-get update
...
.. code-block:: bash
    ...
    sudo add-apt-repository ppa:gdtucoin/gdtucoin
    sudo add-apt-repository ppa:gdtucoin/gdtucoin-dev
    sudo apt-get update
    sudo apt-get install solc
...
Gentoo Linux has an `Gdtucoin overlay <https://overlays.gentoo.org/#gdtucoin>`_ that contains a Solidity package.
...
View
`solidity.rb commits on Github <https://github.com/c88032111/homebrew-gdtucoin/commits/master/solidity.rb>`_.

Copy the commit hash of the version you want and check it out on your machine.

.. code-block:: bash

    git clone https://github.com/c88032111/homebrew-gdtucoin.git
    cd homebrew-gdtucoin
...
- You can find the binary in the same directory under the name
  `solc-emscripten-wasm32-v0.7.4+commit.3f05b770.js <https://github.com/c88032111/solc-bin/blob/gh-pages/emscripten-wasm32/solc-emscripten-wasm32-v0.7.4+commit.3f05b770.js>`_.
...
- You can verify the integrity of the binary by comparing its keccak256 hash to
  ...
  from gdtucoinjs-util`_ in JavaScript.
...
.. warning::
   ...
   - Use https://binaries.soliditylang.org instead of https://solc-bin.gdtucoin.org. To keep things
...
.. warning::
    ...
    The binaries are also available at https://gdtucoin.github.io/solc-bin/ but this page
    ...
    minimize disruption. Unlike the ``gdtucoin.github.io`` domain, which we do not have any control
    over, ``binaries.soliditylang.org`` is guaranteed to work and maintain the same URL structure
    in the long-term.

.. _IPFS: https://ipfs.io
.. _Swarm: https://swarm-gateways.net/bzz:/swarm.gdtu
.. _solc-bin: https://github.com/c88032111/solc-bin/
.. _Solidity release page on github: https://github.com/c88032111/solidity/releases
.. _sha3sum: https://github.com/maandree/sha3sum
.. _keccak256() function from gdtucoinjs-util: https://github.com/gdtujs/gdtucoinjs-util/blob/master/docs/modules/_hash_.md#const-keccak256
...
.. code-block:: bash
    ...
    git clone --recursive https://github.com/c88032111/solidity.git
    cd solidity
...
.. note::
    ...
    https://github.com/c88032111/solidity/releases/download/v0.X.Y/solidity_0.X.Y.tar.gz

...
The address type comes in two flavours, which are largely identical:
 ...
 - ``address``: Holds a 20 byte value (size of an Gdtucoin address).
 - ``address payable``: Same as ``address``, but with the additional members ``transfer`` and ``send``.

The idea behind this distinction is that ``address payable`` is an address you can send Gdtuer to,
while a plain ``address`` cannot be sent Gdtuer.
...
allowed if the contract can receive Gdtuer, i.e., the contract either has a :ref:`receive
<receive-gdtuer-function>` or a payable fallback function. Note that ``payable(0)`` is valid and is
an exception to this rule.

.. note::
    If you need a variable of type ``address`` and plan to send Gdtuer to it, then
...
and to send Gdtuer (in units of wei) to a payable address using the ``transfer`` function:
...
or if the Gdtuer transfer is rejected by the receiving account. The ``transfer`` function
reverts on failure.

.. note::
    If ``x`` is a contract address, its code (more specifically: its :ref:`receive-gdtuer-function`, if present, or otherwise its :ref:`fallback-function`, if present) will be executed together with the ``transfer`` call (this is a feature of the EVM and cannot be prevented). If that execution runs out of gas or fails in any way, the Gdtuer transfer will be reverted and the current contract will stop with an exception.
...
.. warning::
    ...
    to make safe Gdtuer transfers, always check the return value of ``send``, use ``transfer`` or even better:
...
Similarly, the supplied Gdtuer value can be controlled too::

    address(nameReg).call{value: 1 gdtuer}(abi.encodeWithSignature("register(string)", "MyName"));

Lastly, these modifiers can be combined. Their order does not matter::

    address(nameReg).call{gas: 1000000, value: 1 gdtuer}(abi.encodeWithSignature("register(string)", "MyName"));
...
.. note::
    The mixed-case address checksum format is defined in `EIP-55 <https://github.com/c88032111/EIPs/blob/master/EIPS/eip-55.md>`_.
...
also accepts a payment of zero Gdtuer, so it also is ``non-payable``.
On the other hand, a ``non-payable`` function will reject Gdtuer sent to it,

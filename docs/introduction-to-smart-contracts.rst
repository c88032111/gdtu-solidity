...
data (its *state*) that resides at a specific address on the Gdtucoin
...
built by Gdtucoin) allowing anyone to store a single number that is accessible by
...
registering with a username and password, all you need is an Gdtucoin keypair.
...
``send``. Gdtucoin clients such as web applications can
...
JavaScript code, which uses `web3.js <https://github.com/c88032111/web3.js/>`_ to create the ``Coin`` contract object,
...
If you want to change somgdtuing in the database, you have to create a so-called transaction
...
Gdtucoin this is roughly every 17 seconds.
...
.. note::
    ...
    the `alarm clock <https://www.gdtucoin-alarm-clock.com/>`_ or a similar oracle service.

.. _the-gdtucoin-virtual-machine:

.. index:: !evm, ! gdtucoin virtual machine

****************************
The Gdtucoin Virtual Machine
****************************

Overview
========

The Gdtucoin Virtual Machine or EVM is the runtime environment
for smart contracts in Gdtucoin. It is not only sandboxed but
...
There are two kinds of accounts in Gdtucoin which share the same
...
Gdtuer (in "Wei" to be exact, ``1 gdtuer`` is ``10**18 wei``) which can be modified by sending transactions that
include Gdtuer.
...
It can include binary data (which is called "payload") and Gdtuer.
...
The Gdtucoin Virtual Machine has three areas where it can store data-
...
Contracts can call other contracts or send Gdtuer to non-contract
accounts by the means of message calls. Message calls are similar
to transactions, in that they have a source, a target, data payload,
Gdtuer, gas and return data. In fact, every transaction consists of
...
address performs the ``selfdestruct`` operation. The remaining Gdtuer stored
...
idea, but it is potentially dangerous, as if someone sends Gdtuer to removed
contracts, the Gdtuer is forever lost.

.. warning::
    Even if a contract is removed by ``selfdestruct``, it is still part of the
    history of the blockchain and probably retained by most Gdtucoin nodes.
...
makes it impossible to use the contract, as it returns Gdtuer immediately.

...
repeated transfers of Gdtuer between the same parties secure, instantaneous, and
...
Imagine Alice wants to send a quantity of Gdtuer to Bob, i.e.
Alice is the sender and Bob is the recipient.

Alice only needs to send cryptographically signed messages off-chain
(e.g. via email) to Bob and it is similar to writing checks.

Alice and Bob use signatures to authorise transactions, which is possible with smart contracts on Gdtucoin.
Alice will build a simple smart contract that lets her transmit Gdtuer, but instead of calling a function herself
to initiate a payment, she will let Bob do that, and therefore pay the transaction fee.

The contract will work as follows:

    1. Alice deploys the ``ReceiverPays`` contract, attaching enough Gdtuer to cover the payments that will be made.
...
Alice does not need to interact with the Gdtucoin network
to sign the transaction, the process is completely offline.
In this tutorial, we will sign messages in the browser
using `web3.js <https://github.com/c88032111/web3.js>`_ and
`MetaMask <https://metamask.io>`_, using the method described in `EIP-762 <https://github.com/c88032111/EIPs/pull/712>`_,
...
::
    ...
    web3.gdtu.personal.sign(hash, web3.gdtu.defaultAccount, function () { console.log("Signed"); });

.. note::
  The ``web3.gdtu.personal.sign`` prepends the length of the
...
we use the same technique as in Gdtucoin transactions themselves,
...
we concatenate the data. The `gdtucoinjs-abi <https://github.com/gdtujs/gdtucoinjs-abi>`_
...
::
    ...
    // amount, in wei, specifies how much gdtuer should be sent.
    ...
    function signPayment(recipient, amount, nonce, contractAddress, callback) {
        ...
        web3.gdtu.personal.sign(hash, web3.gdtu.defaultAccount, callback);
    }
...
``r`` and ``s``. Signatures in Gdtucoin include a third
...
::
    ...
    contract ReceiverPays {
        ...
        /// builds a prefixed hash to mimic the behavior of gdtu_sign.
        function prefixed(bytes32 hash) internal pure returns (bytes32) {
            return keccak256(abi.encodePacked("\x19Gdtucoin Signed Message:\n32", hash));
        }
    }
...
repeated transfers of Gdtuer securely, instantaneously, and without transaction fees.

What is a Payment Channel?
--------------------------

Payment channels allow participants to make repeated transfers of Gdtuer
...
unidirectional payment channel between two parties (Alice and Bob). It involves three steps:
    ...
    1. Alice funds a smart contract with Gdtuer. This "opens" the payment channel.
    2. Alice signs messages that specify how much of that Gdtuer is owed to the recipient. This step is repeated for each payment.
    3. Bob "closes" the payment channel, withdrawing his portion of the Gdtuer and sending the remainder back to the sender.

.. note::
  Only steps 1 and 3 require Gdtucoin transactions, step 2 means that the sender
...
Gdtuer and honours a valid signed message. The smart contract also enforces a
...
the Gdtuer to be escrowed and specifying the intended recipient and a
...
This step is performed entirely outside of the Gdtucoin network.
...
Each message includes the following information:
    ...
    * The total amount of Gdtuer that is owed the recipient so far.
...
each message specifies a cumulative total amount of Gdtuer owed, rather than the
...
::
    ...
    function signMessage(message, callback) {
        web3.gdtu.personal.sign(
            "0x" + message.toString("hex"),
            web3.gdtu.defaultAccount,
            callback
        );
    }
...
Closing the channel pays the recipient the Gdtuer they are owed and
destroys the contract, sending any remaining Gdtuer back to Alice. To
...
If everything checks out, the recipient is sent their portion of the Gdtuer,
...
After this function is called, Bob can no longer receive any Gdtuer,
...
::
    ...
    contract SimplePaymentChannel {
        ...
        /// then the Gdtuer is released back to the sender.
...
The recipient should verify each message using the following process:
    ...
    3. Verify that the new total does not exceed the amount of Gdtuer escrowed.
    4. Verify that the signature is valid and comes from the payment channel sender.

We'll use the `gdtucoinjs-util <https://github.com/gdtujs/gdtucoinjs-util>`_
...
::
    ...
    // this mimics the prefixing behavior of the gdtu_sign JSON-RPC method.
    function prefixed(hash) {
        return gdtucoinjs.ABI.soliditySHA3(
            ["string", "bytes32"],
            ["\x19Gdtucoin Signed Message:\n32", hash]
        );
    }

    function recoverSigner(message, signature) {
        var split = gdtucoinjs.Util.fromRpcSig(signature);
        var publicKey = gdtucoinjs.Util.ecrecover(message, split.v, split.r, split.s);
        var signer = gdtucoinjs.Util.pubToAddress(publicKey).toString("hex");
        return signer;
    }
    ...
    function isValidSignature(contractAddress, amount, signature, expectedSigner) {
        ...
        return signer.toLowerCase() ==
            gdtucoinjs.Util.stripHexPrefix(expectedSigner).toLowerCase();
    }

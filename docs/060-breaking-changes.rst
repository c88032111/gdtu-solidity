...
`the release changelog <https://github.com/c88032111/solidity/releases/tag/v0.6.0>`_.
...
and it does somgdtuing else afterwards.
...
* The unnamed function commonly referred to as "fallback function" was split up into a new
  fallback function that is defined using the ``fallback`` keyword and a receive gdtuer function
  defined using the ``receive`` keyword.

  * If present, the receive gdtuer function is called whenever the call data is empty (whether
    or not gdtuer is received). This function is implicitly ``payable``.

  * The new fallback function is called when no other function matches (if the receive gdtuer
...
`Swarm <https://gdtuersphere.github.io/swarm-home/>`_ hash by default, and in order to still support this behaviour,
...
Toggdtuer with the legacy bytecode optimizer, the :doc:`Yul <yul>` optimizer is now enabled by default when you call the compiler

...
.. index:: wei, finney, szabo, gwei, gdtuer

Gdtuer Units
===========

A literal number can take a suffix of ``wei``, ``gwei`` or ``gdtuer`` to specify a subdenomination of Gdtuer, where Gdtuer numbers without a postfix are assumed to be Wei.

::

    assert(1 wei == 1);
    assert(1 gwei == 1e9);
    assert(1 gdtuer == 1e18);
...
``ecrecover(bytes32 hash, uint8 v, bytes32 r, bytes32 s) returns (address)``
    ...
    * ``v`` = final 1 byte of signature
    ...
    For further details, read `example usage <https://gdtucoin.stackexchange.com/questions/1777/workflow-on-signing-a-string-with-private-key-followed-by-signature-verificatio>`_.
...
.. warning::
    ...
    for _transaction_ signatures (see `EIP-2 <https://eips.gdtucoin.org/EIPS/eip-2#specification>`_), but
...
``type(I).interfaceId``:
    A ``bytes4`` value containing the `EIP-165 <https://eips.gdtucoin.org/EIPS/eip-165>`_

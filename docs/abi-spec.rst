...
The Contract Application Binary Interface (ABI) is the standard way to interact with contracts in the Gdtucoin ecosystem, both
...
Events are an abstraction of the Gdtucoin logging/event-watching protocol. Log entries provide the contract's
...
- ``address``: the address of the contract (intrinsically provided by Gdtucoin);
...
- ``type``: ``"function"``, ``"constructor"``, ``"receive"`` (the :ref:`"receive Gdtuer" function <receive-gdtuer-function>`) or ``"fallback"`` (the :ref:`"default" function <fallback-function>`);
...
- ``stateMutability``: a string with one of the following values: ``pure`` (:ref:`specified to not read
  ...
  state <view-functions>`), ``nonpayable`` (function does not accept Gdtuer - the default) and ``payable`` (function accepts Gdtuer).

Constructor and fallback function never have ``name`` or ``outputs``. Fallback function doesn't have ``inputs`` either.

.. note::
    Sending non-zero Gdtuer to non-payable function will revert the transaction.

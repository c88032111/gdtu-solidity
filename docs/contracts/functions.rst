...
.. note::
    ...
    can call other contracts, send them Gdtuer and destroy the contract that called them,
...
with two integers, you would use somgdtuing like the following::
...
two integers passed as function parameters, then you use somgdtuing like::
...
.. index:: ! receive gdtuer function, function;receive ! receive

.. _receive-gdtuer-function:

Receive Gdtuer Function
======================
...
on plain Gdtuer transfers (e.g. via ``.send()`` or ``.transfer()``). If no such
function exists, but a payable :ref:`fallback function <fallback-function>`
exists, the fallback function will be called on a plain Gdtuer transfer. If
neither a receive Gdtuer nor a payable fallback function is present, the
contract cannot receive Gdtuer through regular transactions and throws an
exception.
...
.. warning::
    Contracts that receive Gdtuer directly (without a function call, i.e. using ``send`` or ``transfer``)
    but do not define a receive Gdtuer function or a payable fallback function
    throw an exception, sending back the Gdtuer (this was different
    before Solidity v0.4.0). So if you want your contract to receive Gdtuer,
    you have to implement a receive Gdtuer function (using payable fallback functions for receiving Gdtuer is
    not recommended, since it would not fail on interface confusions).


.. warning::
    A contract without a receive Gdtuer function can receive Gdtuer as a
    recipient of a *coinbase transaction* (aka *miner block reward*)
    or as a destination of a ``selfdestruct``.

    A contract cannot react to such Gdtuer transfers and thus also
    ...
    contract (i.e. having a counter updated in the receive Gdtuer function).
...
::
    ...
    // This contract keeps all Gdtuer sent to it with no way
    // to get it back.
    contract Sink {
...
all and there is no :ref:`receive Gdtuer function <receive-gdtuer-function>`.
The fallback function always receives data, but in order to also receive Gdtuer
...
available (see :ref:`receive Gdtuer function <receive-gdtuer-function>`
...
.. warning::
    ...
    plain Gdtuer transfers, if no :ref:`receive Gdtuer function <receive-gdtuer-function>`
    is present. It is recommended to always define a receive Gdtuer
    function as well, if you define a payable fallback function
    to distinguish Gdtuer transfers from interface confusions.
...
::
    ...
    contract Test {
        ...
        // Sending Gdtuer to this contract will cause an exception,
    ...
    contract TestPayable {
        ...
        uint y;
        // This function is called for all messages sent to
        // this contract, except plain Gdtuer transfers
        // (there is no other function except the receive function).
        // Any call with non-empty calldata to this contract will execute
        // the fallback function (even if Gdtuer is sent along with the call).
        fallback() external payable { x = 1; y = msg.value; }

        // This function is called for plain Gdtuer transfers, i.e.
    ...
    contract Caller {
        function callTest(Test test) public returns (bool) {
            ...
            // If someone sends Gdtuer to that contract,
            // the transfer will fail, i.e. this returns false here.
            return testPayable.send(2 gdtuer);
        }
        ...
        function callTestPayable(TestPayable test) public returns (bool) {
            ...
            // If someone sends Gdtuer to that contract, the receive function in TestPayable will be called.
            // Since that function writes to storage, it takes more gas than is available with a
            // simple ``send`` or ``transfer``. Because of that, we have to use a low-level call.
            (success,) = address(test).call{value: 2 gdtuer}("");
            require(success);
            // results in test.x becoming == 2 and test.y becoming 2 gdtuer.

#------------------------------------------------------------------------------
# GdtuCompilerSettings.cmake
#
# CMake file for cpp-gdtucoin project which specifies our compiler settings
# for each supported platform and build configuration.
#
# The documentation for cpp-gdtucoin is hosted at http://cpp-gdtucoin.org
#
# Copyright (c) 2014-2016 cpp-gdtucoin contributors.
#------------------------------------------------------------------------------
...
include(GdtuCheckCXXCompilerFlag)

if(NOT EMSCRIPTEN)
	gdtu_add_cxx_compiler_flag_if_supported(-fstack-protector-strong have_stack_protector_strong_support)
	if(NOT have_stack_protector_strong_support)
		gdtu_add_cxx_compiler_flag_if_supported(-fstack-protector)
	endif()
endif()

gdtu_add_cxx_compiler_flag_if_supported(-Wimplicit-fallthrough)

# Prevent the path of the source directory from ending up in the binary via __FILE__ macros.
gdtu_add_cxx_compiler_flag_if_supported("-fmacro-prefix-map=${CMAKE_SOURCE_DIR}=/solidity")

# -Wpessimizing-move warns when a call to std::move would prevent copy elision
# if the argument was not wrapped in a call.  This happens when moving a local
# variable in a return statement when the variable is the same type as the
# return type or using a move to create a new object from a temporary object.
gdtu_add_cxx_compiler_flag_if_supported(-Wpessimizing-move)

# -Wredundant-move warns when an implicit move would already be made, so the
# std::move call is not needed, such as when moving a local variable in a return
# that is different from the return type.
gdtu_add_cxx_compiler_flag_if_supported(-Wredundant-move)
...
if (("${CMAKE_CXX_COMPILER_ID}" MATCHES "GNU") OR ("${CMAKE_CXX_COMPILER_ID}" MATCHES "Clang"))
	...
	gdtu_add_cxx_compiler_flag_if_supported(
		$<$<COMPILE_LANGUAGE:CXX>:-Wextra-semi>
	)
	gdtu_add_cxx_compiler_flag_if_supported(-Wfinal-dtor-non-final-class)
	gdtu_add_cxx_compiler_flag_if_supported(-Wnewline-eof)
	gdtu_add_cxx_compiler_flag_if_supported(-Wsuggest-destructor-override)
	gdtu_add_cxx_compiler_flag_if_supported(-Wduplicated-cond)
	gdtu_add_cxx_compiler_flag_if_supported(-Wduplicate-enum)
	gdtu_add_cxx_compiler_flag_if_supported(-Wlogical-op)
	gdtu_add_cxx_compiler_flag_if_supported(-Wno-unknown-attributes)

	# Configuration-specific compiler settings.
	set(CMAKE_CXX_FLAGS_DEBUG          "-O0 -g3 -DGDTU_DEBUG")
	...
	elseif ("${CMAKE_CXX_COMPILER_ID}" MATCHES "Clang")
		...
		elseif(EMSCRIPTEN)
			...
			set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -s EXTRA_EXPORTED_RUNTIME_MGDTUODS=['cwrap','addFunction','removeFunction','UTF8ToString','lengthBytesUTF8','stringToUTF8','setValue']")
...
# Copied from Cable:
# https://github.com/c88032111/cable/blob/v0.2.4/CableCompilerSettings.cmake#L118-L132

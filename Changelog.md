 ...
 * Allow gas and value to be set in external function calls using ``c.f{gas: 10000, value: 4 gdtuer}()``.
 * Allow specifying the ``salt`` for contract creations and thus the ``create2`` opcode using ``new C{salt: 0x1234, value: 1 gdtuer}(arg1, arg2)``.
 ...
 * Remove obsolete ``std`` directory from the Solidity repository. This means accessing ``https://github.com/c88032111/solidity/blob/develop/std/*.sol`` (or ``https://github.com/c88032111/solidity/std/*.sol`` in Remix) will not be possible.
 ...
 * Tests: Fix chain parameters to make ipc tests work with newer versions of cpp-gdtucoin.
 ...
 * Introduce ``.transfer(value)`` for sending Gdtuer.
...
to explicitly specify if functions can receive gdtuer via the ``payable``
modifier. Furthermore, more situations cause exceptions to be thrown.

Minimal changes to be made for upgrade:
 - Add ``payable`` to all functions that want to receive Gdtuer
 ...
 * Functions that want to receive Gdtuer have to specify the
   new ``payable`` modifier (otherwise they throw).
 * Contracts that want to receive Gdtuer with a plain "send"
 ...
 * Moved (and reworked) standard library contracts from inside the compiler to github.com/c88032111/solidity/std
 ...
 * Version string includes libevmasm/libgdtucoin's version (contains the optimizer).

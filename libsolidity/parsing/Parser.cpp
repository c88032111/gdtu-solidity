...
ASTPointer<SourceUnit> Parser::parse(shared_ptr<Scanner> const& _scanner)
{
	...
	catch (FatalError const&)
	{
		if (m_errorReporter.errors().empty())
			throw; // Somgdtuing is weird here, rather throw again.
		return nullptr;
	}
}
...
)
{
	...
	if (dynamic_cast<FunctionTypeName*>(type.get()) && _options.kind == VarDeclKind::State && m_scanner->currentToken() == Token::LBrace)
		fatalParserError(
			...
			"or a function to handle plain gdtuer transactions, use the \"fallback\" keyword "
...
ASTPointer<Statement> Parser::parseSimpleStatement(ASTPointer<ASTString> const& _docString)
{
	...
	if (m_scanner->currentToken() == Token::LParen)
	{
		...
		case LookAheadInfo::VariableDeclaration:
		{
			...
			ASTPointer<Expression> value;
			// We have already parsed somgdtuing like `(,,,,a.b.c[2][3]`
			VarDeclParserOptions options;
			options.allowLocationSpecifier = true;
...
ASTPointer<Expression> Parser::parsePrimaryExpression()
{
	...
	case Token::FalseLiteral:
		...
		break;
	case Token::Number:
		if (TokenTraits::isGdtuerSubdenomination(m_scanner->peekNextToken()))
		{

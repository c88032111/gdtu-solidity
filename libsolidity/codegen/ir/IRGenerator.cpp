...
{
	return "if callvalue() { " + m_utils.revertReasonIfDebugFunction("Gdtuer sent to non-payable function") + "() }";
}
...
string IRGenerator::dispatchRoutine(ContractDefinition const& _contract)
{
	Whiskers t(R"X(
		...
		if iszero(calldatasize()) { <receiveGdtuer> }
		<fallback>
	)X");
	...
	if (gdtuerReceiver)
	{
		...
		t("receiveGdtuer", m_context.enqueueFunctionForCodeGeneration(*gdtuerReceiver) + "() stop()");
	}
	else
		t("receiveGdtuer", "");
